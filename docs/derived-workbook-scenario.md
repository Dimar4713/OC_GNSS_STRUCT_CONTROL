...
This flow applies a previously validated XLS/XLSX spacecraft-state workbook as a **new derived scenario**. The selected source YAML remains immutable.

## Required operator inputs

- source scenario selected in the packaged Preview UI;
- validated `.xls` / `.xlsx` workbook;
...
The child configuration contains the imported `spacecraft_states` and `groups`. Existing perturbations are not invented by the import operation.
...
The import adapter remains a convenience layer; no numerical-authority, force-model, frame, time-scale, integrator, or propagation semantics are changed by this operation.
