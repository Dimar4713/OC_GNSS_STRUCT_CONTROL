...
<div class="card" id="perturbationCard">
  ...
  <p class="hint">Каждая включённая строка требует явных M и закона распределения. Gaussian требует σ; Uniform требует нижнюю и верхнюю границы. Приоритет: КА &gt; группа &gt; плоскость &gt; вся ОГ. Seed обязателен. Цели выбираются только из открытого сценария. Исходный сценарий не перезаписывается.</p>
