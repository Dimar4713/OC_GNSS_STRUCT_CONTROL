...
pre{{white-space:pre-wrap;word-break:break-word;background:#111820;color:#d9e2ec;padding:12px;border-radius:6px;max-height:360px;overflow:auto}} .status{{padding:9px;border-radius:6px;background:#eef2f5;margin-top:10px}} .danger{{background:#ffe8e8}} .ok{{background:#e8f7ed}} a.result{{display:block;margin-top:10px;font-weight:700}} .hint{{font-size:12px;color:#566573}} .geometry-plane{{margin:8px 0;padding:8px;background:#f7f9fa;border-radius:6px}} .oplinks{{display:flex;gap:14px;flex-wrap:wrap;margin-top:10px}} .oplinks a{{font-weight:700}}
...
<div class="card"><h3 data-i18n="operations"></h3><p class="hint" data-i18n="operationsHint"></p><div id="operations"></div><div id="oplinks" class="oplinks"></div></div>
...
function renderLanguage(){{document.getElementById('subtitle').textContent=tr('subtitle');document.querySelectorAll('[data-i18n]').forEach(e=>e.textContent=tr(e.dataset.i18n));document.getElementById('openBtn').textContent=tr('open');document.getElementById('runBtn').textContent=tr('run');if(!current)document.getElementById('title').textContent=tr('select');renderOther();renderCurrent();renderOperations();}}
...
function renderOperations(){{const e=document.getElementById('operations');const links=document.getElementById('oplinks');if(!operations){{e.innerHTML=`<p class="hint">${{tr('runToSee')}}</p>`;links.innerHTML='';return}}if(!operations.available){{e.innerHTML=`<p>${{tr('noOperations')}}</p>`;links.innerHTML='';return}}let h=`<div class="table-wrap"><table><tr><th>${{tr('pair')}}</th><th>${{tr('deltaU')}}</th><th>${{tr('driftDay')}}</th><th>${{tr('driftYear')}}</th><th>${{tr('deltaS')}}</th><th>${{tr('alongRate')}}</th><th>${{tr('corridor')}}</th><th>${{tr('state')}}</th><th>${{tr('boundary')}}</th><th>${{tr('timeBoundary')}}</th></tr>`;for(const x of operations.pairs){{const state=x.inside_corridor?tr('inside'):tr('outside');h+=`<tr><td>${{esc(x.pair_id)}}</td><td>${{fmt(x.final_delta_u_deg,4)}}</td><td>${{fmt(x.drift_deg_day,6)}}</td><td>${{fmt(x.drift_deg_julian_year,4)}}</td><td>${{fmt(x.final_along_track_proxy_km,3)}}</td><td>${{fmt(x.along_track_proxy_rate_m_s,6)}}</td><td>${{fmt(x.corridor_half_width_deg,4)}}</td><td><b>${{esc(state)}}</b></td><td>${{fmt(x.predicted_boundary_deg,4)}}</td><td>${{fmt(x.time_to_boundary_days,3)}}</td></tr>`}}h+='</table></div>';h+=`<p class="hint">${{esc(operations.pairs[0].phase_semantics)}}<br>${{esc(operations.pairs[0].along_track_semantics)}}</p>`;e.innerHTML=h;if(artifactLinks){{links.innerHTML=`<a target="_blank" href="${{artifactLinks.phase_plot}}">${{tr('phasePlot')}}</a><a target="_blank" href="${{artifactLinks.along_track_plot}}">${{tr('alongPlot')}}</a><a target="_blank" href="${{artifactLinks.interactive_phase}}">${{tr('interactivePhase')}}</a>`}}else links.innerHTML=''}}
