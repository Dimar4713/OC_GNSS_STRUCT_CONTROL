...
from constellation_control.analysis.campaign_hard_margins import CampaignTrajectoryHardMargins, reduce_trajectory_hard_margins
...
def _initial_request(scenario_path: Path) -> PropagationRequest:
...
def _run_baseline(
    *,
    ...
    replay_request = _full_horizon_replay_request(initial, campaign, profile, profile.controlled_deputy_id)
    ...
    if any(not item.passed for item in hard_constraints):
        raise ValueError(f"authoritative baseline {policy.value} violates a declared hard constraint")
    strategy = OperationalStrategyEvaluation(
...
def run_authoritative_p2_baselines(
    ...
    *,
    ...
    if len({item.strategy.identity for item in baselines}) != 1:
        ...
        "candidates": [
            {
                ...
                "stage": item.stage,
                "parameters": {
                    "trigger_fraction": item.parameters.trigger_fraction,
                    "target_fraction": item.parameters.target_fraction,
                },
