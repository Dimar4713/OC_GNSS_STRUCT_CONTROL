...
def _strategy_plans(
    ...
    plans[optimized.outcome.strategy_id if hasattr(optimized.outcome, "strategy_id") else ""] = ()
    return plans
...
def _strategy_plan_mapping(
    ...
    if authority.evaluation.strategy_id != authority.selection.candidate_id:
        # Strategy id and candidate id are intentionally distinct namespaces in some studies.
        pass
    plans[authority.evaluation.strategy_id] = _ledger_plan(optimized.campaign, controlled_id)
    expected = {
        item.strategy.strategy_id for item in foundation.baselines
    } | {authority.evaluation.strategy_id}
...
def _validate_template(
    ...
    from constellation_control.optimization.operational_robustness import uncertainty_sampling_model_sha256
    ...
    for plan in plans.values():
        for maneuver in plan:
            if maneuver.satellite_id not in {sat.satellite_id for sat in scenario.constellation.satellites}:
...
def _resource_metrics(
    ...
    constraints_reserve_fraction: float,
    controlled_deputy_id: str,
    ...
    for satellite_id, satellite in by_id.items():
        ...
        reserve = satellite.spacecraft.propellant_mass_kg * constraints_reserve_fraction
...
def _sample_executor(
    ...
    *,
    ...
    preflight_reference_id = next(
        satellite.reference_id
        for satellite in scenario.constellation.satellites
        if satellite.satellite_id == profile.controlled_deputy_id
    )
    if preflight_reference_id is None:
        raise ValueError("paired robustness controlled deputy requires reference_id")
    ...
    def execute(sample: Mapping[str, object]) -> CompletedOperationalRealization:
        ...
        margins = reduce_trajectory_hard_margins(
            ...
            reference_id=preflight_reference_id,
...
def run_preview_paired_fixed_plan_robustness(
    ...
    *,
    ...
    re-optimized or re-authorized inside a realization.
    """
    ...
    _validate_template(scenario, profile, foundation, config, plans)
